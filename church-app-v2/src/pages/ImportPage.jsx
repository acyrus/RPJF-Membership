...
export default function ImportPage({ profile, onImportComplete }) {
  ...
  async function importMembers() {
    ...
    let added = 0, updated = 0, skipped = 0, duplicates = 0, errorSkipped = 0, errors = [];
    ...
    memberRows.forEach((row, i) => {
      ...
      dedupeMap.set(key, { row, rowNum: i + 2 });
    });
    ...
    for (const { row, rowNum } of importRows) {
      const get = col => memberMapping[col] ? (row[memberMapping[col]] || "").trim() : "";
      const first = get("first_name"); const last = get("last_name");
      if (!first || !last) { skipped++; continue; }
      // Skip-bad-rows: any row with blocking validation issues is left out; the rest import.
      if (memberRowChecks(row).issues.length > 0) { errorSkipped++; continue; }
      ...
      try {
        ...
        // Check if member already exists by first + last + MIDDLE name.
        // Fetch first+last candidates, then match middle in JS so blank/null
        // middles compare correctly (blank matches blank).
        ...
        if (existing) {
          if (memberReplaceMode) {
            ...
            updated++;
            updatedList.push({ row: rowNum, name: `${first} ${last}` });
          } else {
            // Skip duplicate
            duplicates++;
            continue;
          ...
          memberId = member.id;
          added++;
          addedList.push({ row: rowNum, name: `${first} ${last}` });
        }
    ...
    const result = { added, updated, skipped, duplicates, deduped: dedupedAway, errorSkipped, addedList, updatedList, errors: errors.slice(0, 10), replaced: memberReplaceMode };
